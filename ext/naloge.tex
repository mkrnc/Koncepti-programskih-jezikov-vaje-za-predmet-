...
\end{ex}
\begin{ex}
Unimodalno zaporedje je vsako monotono nara\v s\v cujo\v ce zaporedje \v stevil,
kateremu sledi monotono padajo\v ce zaporedje \v stevil.
Katerokoli od obeh pod-zaporedij je lahko prazno.

Napi\v si funkcijo  
\lstinline{is_unimodal : int list -> bool}
ki kot vhod vzame seznam \v stevil ter odgovori 
pritrdilno \v ce je zaporedje \v stevil v vhodnem seznamu 
unimodalno.
\begin{sol}
\begin{lstlisting}
let rec is_unimodal lst =  match lst with
| [_] -> true
| h1::h2::tl when h1<=h2 -> is_unimodal (h2::tl)
| _ -> let rec is_decr ls = (match ls with 
  | [_] -> true 
  | h1::h2::tl when h1>=h2 -> is_decr (h2::tl)
  | _ -> false)
in is_decr lst
\end{lstlisting}
\end{sol}
...
\begin{ex}
  ...
  pojavijo samo znaka \lstinline{'a'} in \lstinline{'b'}. 
...
\begin{ex}
  Napi\v si funkcijo 
  \lstinline{cikli : int -> int -> int}, ki za klic 
  \lstinline{cikli m n} 
...
\begin{ex}
  ...
  \lstinline{'a} in kazalec na naslednji element seznama tipa \lstinline{'a seznam}, ki je bodisi prazen ali pa ima \v se vsaj en element.
