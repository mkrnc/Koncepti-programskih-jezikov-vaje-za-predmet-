...
type izraz = 
    ...
    | Oper of char * izraz ;; 
\end{lstlisting}
Izraz vsebuje aritmeti\v cne izraze, ki so lahko sestavljeni iz \v stevil (Stevilo) in operacij (Oper). Dovoljene operacije so plus \lstinline{+} in minus \lstinline{-}. Predpostavljamo, da izrazi opisujejo pravilne aritmeti\v cne izraze.
Kot primer navedemo izraz $e = 10 + 5 - 3$, ki ustreza zapisu:
\begin{lstlisting} 
let e = Stevilo (10, Oper ('+', Stevilo (5, Oper ('-', Stevilo (3, Nil)))));;
\end{lstlisting}

Napi\v si funkcijo ovrednoti : \lstinline{izraz -> int}, ki izra\v cuna vrednost izraza.
Primer:
\begin{lstlisting}
eval e;;
- : int = 12
...
\textbf{Dodatna naloga:} Napi\v si funkcijo, ki izpi\v se pravilnostno tabelo za dan boolov izraz.
\end{ex} 
